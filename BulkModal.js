...
// --- BULK MODAL ---
window.BulkModal = ({ isBulkSending, bulkProgress, selectedCount, bulkConfig, setBulkConfig, activeTab, settings, executeBulkSend, close, setShowBulkModal }) => (
    <div className="fixed inset-0 bg-slate-900/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
        <div className="bg-white rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
                    ...
                    <>
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <button onClick={()=>setBulkConfig(p=>({...p, templateType:'AUTO'}))} className={`p-3 rounded-xl border text-left transition-all ${bulkConfig.templateType==='AUTO'?'bg-indigo-600 border-indigo-600 text-white':'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}><div className="font-bold text-sm mb-1">Otomatik Sıradaki</div><div className="text-[10px] opacity-80">Kaldığı aşamadan devam eder</div></button>
                                <button onClick={()=>setBulkConfig(p=>({...p, templateType:'SPECIFIC'}))} className={`p-3 rounded-xl border text-left transition-all ${bulkConfig.templateType==='SPECIFIC'?'bg-indigo-600 border-indigo-600 text-white':'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}><div className="font-bold text-sm mb-1">Tek Tip Şablon</div><div className="text-[10px] opacity-80">Seçilen şablon herkese gider</div></button>
                            </div>
                        </div>
                        {bulkConfig.templateType === 'SPECIFIC' && (<div className="animate-in fade-in"><label className="block text-xs font-bold text-slate-700 mb-1">Şablon Seç</label><select value={bulkConfig.specificStage} onChange={(e)=>setBulkConfig(p=>({...p, specificStage:e.target.value}))} className="w-full p-2 border rounded-lg text-sm bg-slate-50">{settings.workflowTR.map((s,i)=><option key={i} value={i}>{i+1}. {s.label}</option>)}</select></div>)}
                        {activeTab === 'hunter' && (<div><label className="block text-xs font-bold text-slate-700 mb-1">Veritabanı Dili</label><div className="flex gap-2"><button onClick={()=>setBulkConfig(p=>({...p, language:'TR'}))} className={`flex-1 py-1 text-xs font-bold rounded border ${bulkConfig.language==='TR'?'bg-red-50 text-red-600':'bg-white'}`}>Türkçe</button><button onClick={()=>setBulkConfig(p=>({...p, language:'EN'}))} className={`flex-1 py-1 text-xs font-bold rounded border ${bulkConfig.language==='EN'?'bg-blue-50 text-blue-600':'bg-white'}`}>İngilizce</button></div></div>)}
                    </>
                ) : (
            ...
            <div className="p-5 border-t bg-slate-50 flex justify-end">{!isBulkSending && <button onClick={executeBulkSend} className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-xl font-bold text-sm shadow-lg flex items-center gap-2"><window.Icon name="send" className="w-4 h-4"/> Başlat</button>}</div>
        </div>
    </div>
);