...
window.HistoryModal = ({ 
    ...
    // Aktivite Günlüğü (Eskiden kalanlar için boş dizi kontrolü)
    const logs = historyModalLead.activityLog || [];
    
    // Logları orijinal index'leri ile eşleştirip sırala
        ...
        <div className="fixed inset-0 bg-slate-900/70 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
            <div className="bg-white rounded-2xl w-full max-w-lg flex flex-col max-h-[90vh] overflow-hidden shadow-2xl">
                ...
                <div className="p-6 overflow-y-auto custom-scrollbar">
                    ...
                    {/* --- SÜREÇ ZAMAN ÇİZELGESİ (ESKİ SİSTEM - HALA FAYDALI) --- */}
                    <div className="mb-6 pb-6 border-b border-slate-100">
                        ...
                        <div className="relative border-l-2 border-indigo-100 ml-4 space-y-6">
                            ...
                            {(!historyModalLead.history || Object.values(historyModalLead.history).every(d => !d)) && (
                                <div className="pl-6 text-sm text-slate-400 italic">Henüz iş akışı başlamadı.</div>
                            )}
                        </div>
                    </div>

                    {/* --- YENİ: AKTİVİTE GÜNLÜĞÜ VE NOTLAR --- */}
                    <div className="mb-6">
                        ...
                        <div className="space-y-3 max-h-60 overflow-y-auto custom-scrollbar pr-1">
                            {sortedLogs.length > 0 ? sortedLogs.map((log) => (
                                <div key={log.originalIndex} className={`p-3 rounded-lg text-sm border group relative ${log.type === 'MAIL' ? 'bg-blue-50 border-blue-100' : 'bg-yellow-50 border-yellow-100'}`}>
                                    <div className="flex justify-between items-center mb-1">
                                        <span className={`text-[10px] font-bold uppercase px-1.5 py-0.5 rounded ${log.type === 'MAIL' ? 'bg-blue-200 text-blue-800' : 'bg-yellow-200 text-yellow-800'}`}>
                                            {log.type === 'MAIL' ? 'Mail' : 'Not'}
                                        </span>
                                    ...
                                    {editingNoteIndex === log.originalIndex ? (
                                        ...
                                        <div className="text-slate-700 leading-snug break-words pr-6">
                                            {log.content}
                                        </div>
                                    )}

                                    {/* Düzenle/Sil Butonları (Sadece 'NOTE' tipi için ve hover olunca görünür) */}
                                    {log.type === 'NOTE' && editingNoteIndex !== log.originalIndex && (
                                        <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1 bg-white/80 p-1 rounded shadow-sm">
                                            <button onClick={() => startEditing(log)} className="p-1 hover:text-blue-600" title="Düzenle">
                                                <window.Icon name="edit-2" className="w-3 h-3"/>
                                            </button>
                                            <button onClick={() => onDeleteNote(historyModalLead.id, log.originalIndex)} className="p-1 hover:text-red-600" title="Sil">
                                ...
                                <div className="text-center py-4 text-xs text-slate-400 border border-dashed rounded-lg">
                                    Henüz kayıtlı bir not veya mail logu yok.
                                </div>
                            )}
                        </div>
                    </div>
