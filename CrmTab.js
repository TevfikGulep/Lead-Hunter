...
// Bağımlılıklar: FilterBar, PaginationControls, SortIcon, Icon, cleanDomain gereklidir.
...
window.CrmTab = ({
    ...
    onBulkCheck,
    isCheckingBulk,
    paginatedItems, // getPaginatedData() sonucu
    selectedCount,
    toggleSelectAll,
    toggleSelection,
    handleSort,
    sortConfig, // SortIcon için gerekli
    editingRowId,
    editFormData,
    handleEditChange,
    handleEditSave,
    ...
    emailMap,
    getStageInfo,
    enrichDatabase, // Yeni prop: Zenginleştir fonksiyonu
    isEnriching,    // Yeni prop
    setShowImportModal, // Yeni prop: Import modali aç
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden animate-in fade-in">
            <div className="p-6 border-b border-slate-100 flex flex-col gap-4">
                <div className="flex flex-wrap justify-between items-center gap-4">
                    ...
                    <div className="flex gap-2">
                        <button 
                            ...
                            className="bg-indigo-50 text-indigo-700 hover:bg-indigo-100 px-3 py-1.5 rounded-lg text-xs font-bold border border-indigo-200 flex items-center gap-2 transition-colors"
                        >
                            <window.Icon name="upload-cloud" className="w-4 h-4"/> Liste Yükle
                        </button>
                        
                        {/* ZENGİNLEŞTİRME SEÇİM MENÜSÜ */}
                        <div className="relative group">
                            <button 
                                disabled={isEnriching}
                                className="bg-purple-50 text-purple-700 hover:bg-purple-100 px-3 py-1.5 rounded-lg text-xs font-bold border border-purple-200 flex items-center gap-2 transition-colors disabled:opacity-50"
                            >
                            ...
                            {!isEnriching && (
                                <div className="absolute right-0 top-full mt-1 w-48 bg-white border border-slate-200 rounded-lg shadow-xl z-50 hidden group-hover:block animate-in fade-in zoom-in-95 duration-100">
                                    <div className="p-1">
                                        <button onClick={() => enrichDatabase('EMAIL')} className="w-full text-left px-3 py-2 text-xs font-medium text-slate-700 hover:bg-purple-50 hover:text-purple-700 rounded-md transition-colors flex items-center gap-2">
                                            <window.Icon name="mail" className="w-3 h-3"/> Eksik Mailleri Bul
                                        </button>
                                        <button onClick={() => enrichDatabase('TRAFFIC')} className="w-full text-left px-3 py-2 text-xs font-medium text-slate-700 hover:bg-purple-50 hover:text-purple-700 rounded-md transition-colors flex items-center gap-2">
                                            <window.Icon name="bar-chart-2" className="w-3 h-3"/> Eksik Trafiği Bul
                                        </button>
                                        <div className="h-px bg-slate-100 my-1"></div>
                                        <button onClick={() => enrichDatabase('BOTH')} className="w-full text-left px-3 py-2 text-xs font-bold text-purple-700 hover:bg-purple-50 rounded-md transition-colors flex items-center gap-2">
                                            <window.Icon name="check-circle-2" className="w-3 h-3"/> Tümünü Tara (Full)
                                        </button>
                                    </div>
                                </div>
                            )}
                ...
                <window.FilterBar 
                    ...
                    onBulkStatusChange={bulkUpdateStatus} // FilterBar'a fonksiyonu iletiyoruz
                />
            </div>
                ...
                <table className="w-full text-left text-sm">
                    <thead className="bg-slate-50 text-slate-500 border-b">
                        <tr>
                            ...
                            <th className="p-4 cursor-pointer" onClick={() => handleSort('url')}>Site</th>
                            <th className="p-4">Email</th>
                            <th className="p-4">Son Gönderilen</th>
                            <th className="p-4" onClick={() => handleSort('lastContactDate')}>
                                <div className="flex items-center gap-1 cursor-pointer hover:text-indigo-600">Son Temas <window.SortIcon column="lastContactDate" sortConfig={sortConfig}/></div>
                            </th>
                            <th className="p-4" onClick={() => handleSort('potential')}>
                                <div className="flex items-center gap-1 cursor-pointer hover:text-indigo-600">Potansiyel <window.SortIcon column="potential" sortConfig={sortConfig}/></div>
                            </th>
                            <th className="p-4">Notlar</th>
                            <th className="p-4">Dil</th>
                            <th className="p-4 cursor-pointer" onClick={() => handleSort('statusKey')}>
                                <div className="flex items-center gap-1 hover:text-indigo-600">Durum <window.SortIcon column="statusKey" sortConfig={sortConfig}/></div>
                            </th>
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {paginatedItems.map(item => {
                            ...
                            <tr key={item.id} className={`hover:bg-slate-50 transition-colors ${editingRowId === item.id ? 'bg-indigo-50' : ''}`}>
                                ...
                                <td className="p-4 text-sm text-slate-600 max-w-[200px]">
                                    {editingRowId === item.id ? <input value={editFormData.email} onChange={e => handleEditChange('email', e.target.value)} className="w-full p-1 border rounded" /> : (
                                        <div className="flex flex-col">
                                            ...
                                            {siteCount > 1 && (
                                                <span 
                                                    className="text-[10px] font-bold text-purple-600 bg-purple-50 px-1.5 py-0.5 rounded-full w-fit mt-1 cursor-pointer hover:bg-purple-100"
                                                    title={`Diğer Siteler: ${relatedSites.join(', ')}`}
                                                    onClick={(e) => { e.stopPropagation(); alert(`Bu E-posta Adresine Bağlı Siteler:\n\n${relatedSites.join('\n')}`); }}
                                                >
                                ...
                                <td className="p-4 text-slate-500 text-xs max-w-[150px] truncate" title={item.notes}>
                                    {editingRowId === item.id ? <input value={editFormData.notes} onChange={e => handleEditChange('notes', e.target.value)} className="w-full p-1 border rounded" /> : item.notes || '-'}
                                </td>
                                <td className="p-4">
                                    {editingRowId === item.id ? <select value={editFormData.language} onChange={e => handleEditChange('language', e.target.value)} className="p-1 border rounded"><option value="TR">TR</option><option value="EN">EN</option></select> : <span className={`text-[10px] px-2 py-1 rounded border ${item.language === 'TR' ? 'bg-red-50 border-red-200 text-red-700' : 'bg-blue-50 border-blue-200 text-blue-700'}`}>{item.language || 'TR'}</span>}
                                </td>
                                <td className="p-4">
                                    {editingRowId === item.id ? (
                                        <select 
                                            value={editFormData.statusKey} 
                                            onChange={e => handleEditChange('statusKey', e.target.value)} 
                                            className="p-1 border rounded text-xs max-w-[120px]"
                                        >
                                            <option value="New">New</option>
                                            {Object.keys(window.LEAD_STATUSES).map(statusKey => (
                                                <option key={statusKey} value={statusKey}>
                                                    {window.LEAD_STATUSES[statusKey].label}
                                                </option>
                                            ))}
                                        </select>
                                    ) : (
                                        <span className={`px-2 py-1 rounded text-xs font-bold border ${window.LEAD_STATUSES[item.statusKey]?.color || 'bg-gray-100'}`}>{window.LEAD_STATUSES[item.statusKey]?.label || item.statusLabel}</span>
                                    )}
                                </td>
