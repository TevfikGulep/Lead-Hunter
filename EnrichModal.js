        ...
        <div className="bg-white rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 space-y-4 overflow-y-auto flex-1">
                <div className="text-center">
                    <div className="text-2xl font-bold text-purple-600 mb-1">%{Math.round((enrichProgress.current/enrichProgress.total)*100)}</div>
                    <div className="text-xs text-slate-500">{enrichProgress.current} / {enrichProgress.total} Site Tarandı</div>
                </div>
                <div className="w-full bg-slate-100 rounded-full h-2 overflow-hidden">
                    <div className="bg-purple-600 h-full transition-all duration-300" style={{width: `${(enrichProgress.current/enrichProgress.total)*100}%`}}></div>
                </div>
