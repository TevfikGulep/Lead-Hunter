...
// config.js'den gelen Firebase ayarını string'e çevir
    ...
    const processAndSetCrmData = (rawData) => {
        const terminalStatuses = ['DEAL_ON', 'DENIED', 'NOT_VIABLE', 'MAIL_ERROR', 'NON_RESPONSIVE', 'NOT_POSSIBLE'];
        const map = {}; 
    ...
    const processedData = useMemo(() => {
        ...
        return data;
    }, [crmData, filters, sortConfig]);

    const processedHunterLeads = useMemo(() => {
        let data = [...leads];
        if (hunterFilterType === 'VIABLE') { data = data.filter(l => l.trafficStatus && l.trafficStatus.viable); } 
        else if (hunterFilterType === 'LOW') { data = data.filter(l => !l.trafficStatus || !l.trafficStatus.viable); }

        data.sort((a, b) => {
            let valA = 0, valB = 0;
            if (hunterSort.key === 'traffic') { valA = a.trafficStatus?.value || 0; valB = b.trafficStatus?.value || 0; }
            if (hunterSort.direction === 'asc') return valA - valB;
            return valB - valA;
        });
        return data;
    }, [leads, hunterSort, hunterFilterType]);
    ...
    const checkGmailReply = async (lead) => {
        ...
        try {
            ...
            if (data.status === 'success') {
                ...
                if (data.isBounce && isDbConnected && confirm("BU MAİL HATALI (BOUNCE)!\n\nKayıt güncellensin mi?")) {
                    await dbInstance.collection("leads").doc(lead.id).update({ statusKey: 'MAIL_ERROR', statusLabel: 'Error in mail (Bounced)', email: '', lastContactDate: new Date().toISOString(), notes: (lead.notes || '') + ' [Sistem: Hatalı Mail Silindi]' });
                    alert("Kayıt güncellendi."); setHistoryModalLead(null); 
                }
            ...
            if (data.status === 'success') {
                ...
                candidates.forEach(lead => {
                    ...
                    if (result && result.hasReply) {
                        ...
                        if (result.isBounce) {
                            batch.update(ref, { statusKey: 'MAIL_ERROR', statusLabel: 'Error in mail (Bounced)', email: '', lastContactDate: new Date().toISOString() });
                            bounceCount++; hasUpdates = true;
                        } else if (!['INTERESTED', 'DEAL_ON', 'NOT_POSSIBLE', 'DENIED', 'MAIL_ERROR'].includes(lead.statusKey)) {
                            batch.update(ref, { statusKey: 'INTERESTED', statusLabel: 'Showed interest (Reply Found)', lastContactDate: new Date().toISOString() });
    ...
    const enrichDatabase = async () => {
        const targets = crmData.filter(item => (!item.email || item.email.length<5 || !item.trafficStatus?.label || item.statusKey==='MAIL_ERROR') && !['NOT_VIABLE'].includes(item.statusKey));
        if (targets.length === 0) return alert("Veriler tam.");
        setShowEnrichModal(true); setIsEnriching(true); setEnrichLogs([]); setEnrichProgress({ current: 0, total: targets.length });
        
        for (let i = 0; i < targets.length; i++) {
            const lead = targets[i]; let updates = {};
            setEnrichProgress(prev => ({ ...prev, current: i + 1 }));
            if (!lead.trafficStatus?.label || lead.trafficStatus.label === 'Bilinmiyor') {
                try { const t = await window.checkTraffic(lead.url); if(t && t.label!=='Hata') updates.trafficStatus = t; } catch(e){}
            }
            if (!lead.email || lead.email.length < 5 || lead.statusKey === 'MAIL_ERROR') {
                try { const e = await window.findEmailsOnSite(lead.url); if(e) { updates.email = e; if(lead.statusKey==='MAIL_ERROR') { updates.statusKey='New'; updates.stage=0; } } } catch(err){}
            }
            if (Object.keys(updates).length > 0 && isDbConnected) {
                await dbInstance.collection("leads").doc(lead.id).update(updates);
                setEnrichLogs(p=>[...p, {time: new Date().toLocaleTimeString(), msg: `${lead.url} güncellendi`, type: 'success'}]);
        ...
        <div className="flex h-screen bg-slate-50 font-sans text-slate-800 overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto bg-slate-100 relative">
                <div className="w-full mx-auto p-6 md:p-8">
                    ...
                    {activeTab === 'dashboard' && <window.DashboardTab crmData={crmData} filters={filters} setFilters={setFilters} selectedIds={selectedIds} toggleSelection={toggleSelection} toggleSelectAll={toggleSelectAll} selectedCount={selectedIds.size} setShowBulkModal={setShowBulkModal} activeTab={activeTab} fixAllTrafficData={fixAllTrafficData} onBulkCheck={handleBulkReplyCheck} isCheckingBulk={isCheckingBulk} paginatedItems={getPaginatedData()} currentPage={currentPage} totalPages={totalPages} setCurrentPage={setCurrentPage} totalRecords={processedData.length} setHistoryModalLead={setHistoryModalLead} getStageInfo={getStageInfo} />}
